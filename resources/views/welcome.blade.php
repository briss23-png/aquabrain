    ...
    <main>
        ...
        <section id="hero" class="hero-section">
            ...
            <div style="display: none;">
                <img src="{{ asset('img/appareil.webp') }}" loading="lazy" alt="preload">
                <img src="{{ asset('img/boss.webp') }}" loading="lazy" alt="preload">
                <img src="{{ asset('img/apprend.webp') }}" loading="lazy" alt="preload">
                <img src="{{ asset('img/travaille.webp') }}" loading="lazy" alt="preload">
            </div>
    ...
    <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 15px;">
        ...
        <section id="realisations" class="container animate-on-scroll">
            ...
            <div class="map-container">
                <img src="{{ asset('img/carte_afrique.webp') }}" loading="lazy" alt="Carte Afrique" class="map-image">
...
<script>
// DONNÉES DES PROJETS POUR LA MAP
const projectsData = {
    tg: { 
        name: "LOFTY FARM - Togo", 
        desc: "Ferme piscicole moderne - 800 tonnes/an",
        image: "{{ asset('img/lofty.jpg') }}"
    },
    ml: { 
        name: "SEM GROUPE - Mali", 
        desc: "Unité d'alevinage - 1M alevins/mois",
        image: "{{ asset('img/sem.jpg') }}"
    },
    cd: { 
        name: "ACCESSLANDS - RDC", 
        desc: "Complexe d'écloseries - 34 tonnes/cycle",
        image: "{{ asset('img/complex.jpg') }}"
    },
    bj: { 
        name: "Projet Bénin", 
        desc: "Ferme de production - En construction",
        image: "{{ asset('img/benin.jpg') }}"
    }
};

// POSITIONS DES MARQUEURS (en pourcentage)
const markers = [
    { x: 35, y: 45, country: 'tg' },   // Togo
    { x: 28, y: 55, country: 'bj' },   // Bénin
    { x: 18, y: 48, country: 'ml' },   // Mali
    { x: 55, y: 42, country: 'cd' }    // RDC
];

// CRÉER LES MARQUEURS SUR LA MAP
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Créer le conteneur des marqueurs
    let markersContainer = mapContainer.querySelector('.markers-container');
    if (!markersContainer) {
    ...
    if (!tooltip) {
        ...
        tooltip.style.border = '3px solid var(--primary-color)';
    ...
    markers.forEach(marker => {
        ...
        dot.style.backgroundColor = 'var(--gold-color)';
        ...
        const project = projectsData[marker.country];
        
        dot.addEventListener('mouseenter', function(e) {
            tooltip.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    ${project.image ? `<img src="${project.image}" loading="lazy" style="width: 100%; height: 120px; object-fit: cover; border-radius: 8px;">` : ''}
                    <h4 style="color: var(--primary-color); margin: 0;">${project.name}</h4>
                    <p style="margin: 0; font-size: 0.9rem;">${project.desc}</p>
                </div>
            `;
            ...
            updatePosition(e);
            dot.addEventListener('mousemove', updatePosition);
            
            dot.addEventListener('mouseleave', function() {
                tooltip.style.display = 'none';
                dot.removeEventListener('mousemove', updatePosition);
            });
        });
